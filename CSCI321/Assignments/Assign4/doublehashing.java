public class doublehashing<Key, Value> {
    ...
    private int doublehash(Key key) {
        return (key.hashCode() + 2 & 0x7fffffff) % m;
    }
    ...
    public void put(Key key, Value val) {
        ...
        int i;
        for (i = hash(key); keys[i] != null; i = (i + doublehash(key)) % m) {
    ...
    public Value get(Key key) {
        ...
        for (int i = hash(key); keys[i] != null ; i = i + doublehash(key))
            if (keys[i].equals(key))
                return vals[i];
        return null;
    }
    ...
    public static void main(String[] args) {
        doublehashing<String, String> st = new doublehashing<String, String>();
